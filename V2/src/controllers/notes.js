...
class Notes {
    ...
    makeComment(req, res) {
        NoteService.findOne({ _id: req.params.id }).then((mainNote) => {
            if (!mainNote)
                return res
        ...
        NoteService.findOne({ _id: req.params.id }).then((mainNote) => {
            if (!mainNote)
                return res
                    .status(httpStatus.NOT_FOUND)
                    .json({ message: 'Böyle bir not bulunmamaktadır' })
            mainNote.comments = mainNote.comments.filter(
    ...
    listNote(req, res) {
        if (!req.params.id)
            return res
                ...
                .json({ message: 'ID bilgisi gerekli' })
        NoteService.findOne({ _id: req.params.id })
            .then((note) => {
                if (!note)
                    return res
                        .status(httpStatus.NOT_FOUND)
                        .json({ message: 'Böyle bir kayıt bulunamadı.' })
